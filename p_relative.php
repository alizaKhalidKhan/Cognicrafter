     ...
     <div class="container-xxl py-5">
            <div class="container">
                <div class="text-center mx-auto mb-5 wow fadeInUp" data-wow-delay="0.1s" style="max-width: 600px;">
                    <h1 class="mb-3">Relative Information</h1>
                </div>
                <div class="owl-carousel testimonial-carousel wow fadeInUp" data-wow-delay="0.1s" >
                    ...
                    if (isset($_SESSION['user_name'])) {
                        ...
                        if ($patient_result->num_rows > 0) {
                            ...
                            $rell_query = "SELECT Relative_name, Relative_phone , Relative_email , Relative_address FROM relative WHERE Patient_id = $patient_id";
                            $rell_result = $conn->query($rell_query);

                            if ($rell_result === false) {
                                die("Error in Relative query: " . $conn->error);
                            }

                            if ($rell_result->num_rows > 0) {
                                while ($row = $rell_result->fetch_assoc()) {
                                    // Generate HTML for each class item
                                    echo '
                                    <div class="testimonial-item bg-light rounded p-5">
                                        <p class="fs-5">Relative Name :' . $row['Relative_name'] . '</p>
                                        <p class="fs-5">Relative Phone : ' . $row['Relative_phone'] . '</p>
                                        <p class="fs-5">Relative Email : ' . $row['Relative_email'] . '</p>
                                        <p class="fs-5">Relative Address : ' . $row['Relative_address'] . '</p>
                                    </div>';
                                }
                            } else {
                                echo "No relative found for this patient.";
                            }
                        } else {
