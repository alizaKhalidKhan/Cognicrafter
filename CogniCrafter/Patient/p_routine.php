...
<body>
    ...
    <div class="container-xxl bg-white p-0">
        ...
        <div class="container-xxl py-5">
            <div class="container">
                <h1 style="text-align: center;">Routine Activities</h1><br><br>

                <!-- Add a form for filter selection -->
                <form method="get" action="">
                    <select name="filter" onchange="this.form.submit()">
                        <option value="previous" <?php echo ($_GET['filter'] ?? '') == 'previous' ? 'selected' : ''; ?>>Previous</option>
                        <option value="today" <?php echo ($_GET['filter'] ?? '') == 'today' ? 'selected' : ''; ?>>Today</option>
                        <option value="future" <?php echo ($_GET['filter'] ?? '') == 'future' ? 'selected' : ''; ?>>Future</option>
                    </select>
                </form><br><br>
                ...
                <div class="row g-4">
                    ...
                    if (isset($_SESSION['user_name'])) {
                        ...
                        if ($patient_result->num_rows > 0) {
                            ...
                            // Set default filter to 'today' if not selected
                            $filter = isset($_GET['filter']) ? $_GET['filter'] : 'today';

                            // Fetch data from the routine table based on patient_id and filter
                            $routine_query = "SELECT Routine_name, Routine_date, Routine_time, Routine_type FROM routineactivity WHERE Patient_id = $patient_id";

                            // Adjust the query based on the selected filter
                            switch ($filter) {
                                case 'previous':
                                    $routine_query .= " AND Routine_date < CURDATE()";
                                    break;
                                case 'today':
                                    $routine_query .= " AND Routine_date = CURDATE()";
                                    break;
                            ...
                            if ($routine_result === false) {
                                die("Error in routine query: " . $conn->error);
                            }
                            ...
                            if ($routine_result->num_rows > 0) {
                                    ...
                                    <div class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
                                        <div class="classes-item">
                                            <div class="bg-light rounded-circle w-75 mx-auto p-3">
                                                <img class="img-fluid rounded-circle" src="img/todos.jpg" alt="">
                                            </div>
                                            <div class="bg-light rounded p-4 pt-5 mt-n5">
                                                <a class="d-block text-center h3 mt-3 mb-4" href="#">' . $row['Routine_name'] . '</a>
                                                
                                                <div class="row g-1">
                                                    <div class="col-4">
                                                        <div class="border-top border-3 border-primary pt-2">
                                                            <h6 class="text-primary mb-1">Routine date:</h6>
                                                            <small>' . $row['Routine_date'] . '</small>
                                                        </div>
                                                    </div>
                                                    <div class="col-4">
                                                        <div class="border-top border-3 border-success pt-2">
                                                            <h6 class="text-success mb-1">Routine time:</h6>
                                                            <small>' . $row['Routine_time'] . '</small>
                                                        </div>
                                                    </div>
                                                    <div class="col-4">
                                                        <div class="border-top border-3 border-warning pt-2">
                                                            <h6 class="text-warning mb-1">Routine type:</h6>
                                                            <small>' . $row['Routine_type'] . '</small>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                echo "No routine reminders found for this patient.";
                            }
                        } else {
